// Single source of truth for brand assets.
// When the logo files change, update ONLY these constants
// and every reference across the site updates automatically.

export const LOGO_LIGHT = "/MainLogoNew.png";     // dark text — use on light/white backgrounds
export const LOGO_DARK  = "/MainLogoBlueNew.png"; // light text — use on dark/navy backgrounds
export const LOGO_OG    = "/og-image.png";        // 1200x630 branded image for social sharing

export const SITE_URL   = "https://rizexcapital.vercel.app";
